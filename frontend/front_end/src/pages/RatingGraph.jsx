...
const createCombinedData = (historyData) => {
  const sortedData = [...historyData].sort(
    (a, b) => new Date(a.contest_date) - new Date(b.contest_date)
  );
  ...
  for (const event of sortedData) {
    ...
    else if (event.platform === "codeforces")
      lastCodeforcesRating = event.rating;
  ...
  useEffect(() => {
    const fetchRatingHistory = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            "Failed to fetch data. Please ensure you are logged in and the server is running."
          );
        }
